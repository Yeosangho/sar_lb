...
def dqn_learing(
    ...
    def select_epilson_greedy_action(model, obs, t, target_controller_id):
        ...
        if sample > eps_threshold:
            ...
            with torch.no_grad():
                ...
                return actions.data.max(1)[1].cpu()
        else:
    ...
    for i in range(controller_num) :
        ...
        optimizer_ctrl = optim.RMSprop(params=q.parameters(), lr=0.001, alpha=0.95, eps= 0.01)
    ...
    for episode in range(10000):
        ...
        for itr in range(episode_length):
            ...
            if (t[target_controller_id] > learning_starts and
                ...
                qval = qval.reshape(64,3)
                current_Q_values = qval.gather(1, act_batch.unsqueeze(1))
                #print(next_max_qprint(current_Q_values.shape)
                ...
                target_qval = target_qval.reshape(64,3)
