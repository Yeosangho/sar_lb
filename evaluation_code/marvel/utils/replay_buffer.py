...
class ReplayBuffer(object):
    def __init__(self, size, frame_history_len, input_args):
    ...
    def sample(self, batch_size):
        ...
        idxes = sample_n_unique(lambda: random.choice(self.indexes), batch_size)
        #print(idxes)
        return self._encode_sample(idxes)

    def encode_recent_observation(self, validation=False):
        ...
        return self._encode_observation((self.next_idx - 1) % self.size, validation)

    def _encode_observation(self, idx, validation=False):

        end_idx   = idx + 1 # make noninclusive
        start_idx = end_idx - self.frame_history_len

        #print(f"end idx {end_idx}, start_idx {start_idx}")
        ...
        #print(f"missing_context : {missing_context}")
        ...
        if start_idx < 0 or missing_context > 0:
            ...
            return self.obs[start_idx:end_idx].reshape(-1, self.input_args)
